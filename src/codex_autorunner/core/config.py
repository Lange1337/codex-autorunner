...
def maybe_upgrade_generated_hub_pma_defaults(config_path: Path) -> Dict[str, Any]:
    """Upgrade stale generated hub defaults without overriding explicit root config."""
    try:
    ...
    if not isinstance(pma, dict):
        return data
    if pma.get("max_text_chars") != PMA_LEGACY_GENERATED_MAX_TEXT_CHARS:
        return data

    root = config_path.parent.parent.resolve()
    if _root_explicitly_sets_pma_max_text_chars(root):
        return data

    updated = dict(data)
    updated_pma = dict(pma)
    updated_pma["max_text_chars"] = PMA_DEFAULT_MAX_TEXT_CHARS
    updated["pma"] = updated_pma
    rendered = GENERATED_CONFIG_HEADER + yaml.safe_dump(updated, sort_keys=False)
    try:
        atomic_write(config_path, rendered)
    except OSError:
        logger.warning(
            "Failed to persist generated PMA max_text_chars migration for %s",
            config_path,
            exc_info=True,
        )
    return updated
...
def load_hub_config_data(config_path: Path) -> Dict[str, Any]:
    ...
    data = maybe_upgrade_generated_hub_pma_defaults(config_path)
    mode = data.get("mode")
