...
from ..opencode.harness import _coerce_providers, _iter_provider_models
from ..opencode.runtime import (
    build_turn_id,
    extract_session_id,
    extract_turn_id,
    split_model_id,
...
_ALLOWED_PROVIDER_PREFIXES = ("anthropic/", "claude/")


def _filter_model_catalog(catalog: ModelCatalog) -> ModelCatalog:
    filtered = [
        model
        for model in catalog.models
        if isinstance(model.id, str)
        and any(model.id.startswith(prefix) for prefix in _ALLOWED_PROVIDER_PREFIXES)
    ]
    default_model = catalog.default_model
    if default_model and any(
        default_model.startswith(prefix) for prefix in _ALLOWED_PROVIDER_PREFIXES
    ):
        return ModelCatalog(default_model=default_model, models=filtered)
    if filtered:
        return ModelCatalog(default_model=filtered[0].id, models=filtered)
    return ModelCatalog(default_model="", models=[])
...
class ClaudeHarness(AgentHarness):
    ...
    async def model_catalog(self, workspace_root: Path) -> ModelCatalog:
        ...
        return _filter_model_catalog(
            ModelCatalog(default_model=default_model, models=models)
        )
    ...
    async def new_conversation(
        ...
        if not isinstance(session_id, str) or not session_id:
            raise ValueError("Claude did not return a session id")
        return ConversationRef(agent=AgentId("claude"), id=session_id)
        ...
        for entry in sessions:
            ...
            if isinstance(session_id, str) and session_id:
                conversations.append(
                    ConversationRef(agent=AgentId("claude"), id=session_id)
                )
    ...
    async def start_review(
        ...
        *,
        ...
        _ = model, reasoning, approval_mode, sandbox_policy
        client = await self._supervisor.get_client(workspace_root)
        ...
        async def _send_review() -> None:
            try:
                ...
                if turn_id:
                    _logger.debug("Claude review started: %s", turn_id)
            except Exception as exc:
                _logger.warning("Claude review command failed: %s", exc)
    ...
    async def interrupt(
        ...
        client = await self._supervisor.get_client(workspace_root)
        await client.abort_session(conversation_id, turn_id=turn_id)
